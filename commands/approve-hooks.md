...
1. **List hooks**: Run `skillgate claude hooks list --directory .`. Display all hooks with their current status:
...
5. **Approve confirmed hooks**: For each confirmed hook, run `skillgate claude hooks approve <path> --directory .`. Report the attestation record created.
